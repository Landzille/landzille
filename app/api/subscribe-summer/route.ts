...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      if (data.title === "Member Exists") {
        ...
        await fetch(tagsUrl, {
          ...
          body: JSON.stringify({
            tags: [{ name: "Summer 2026", status: "active" }],
          }),
        });
        ...
        return NextResponse.json(
          {
            message:
              "You're already on our list! We'll notify you about Summer 2026.",
          },
          { status: 200 }
        );
      }
      ...
      if (data.title === "Forgotten Email Not Subscribed") {
        return NextResponse.json(
          {
            error:
              "This email was previously unsubscribed. Please use a different email or contact us directly at info@landzille.com to resubscribe.",
          },
          { status: 400 }
        );
      }
    ...
    return NextResponse.json(
      { message: "Successfully joined! We'll notify you about Summer 2026." },
      { status: 200 }
    );
  } catch (error) {
    console.error("Summer waitlist subscription error:", error);
    return NextResponse.json(
